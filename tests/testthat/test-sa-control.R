...
test_that("control_sim_anneal bad arg passing", {
  ...
  expect_snapshot(error = TRUE, control_sim_anneal(restart = "yes"))
  expect_snapshot(error = TRUE, control_sim_anneal(restart = 0:1))
  expect_snapshot(error = TRUE, control_sim_anneal(restart = 1))
  expect_snapshot(control_sim_anneal(no_improve = 2, restart = 6))
  expect_snapshot(error = TRUE, control_sim_anneal(radius = "huge"))
  ...
  expect_snapshot(error = TRUE, control_sim_anneal(flip = 0:1))
  expect_snapshot(error = TRUE, control_sim_anneal(flip = "huge"))
  expect_equal(control_sim_anneal(flip = -1)$flip, 0)
  expect_equal(control_sim_anneal(flip = 2)$flip, 1)
  expect_snapshot(error = TRUE, control_sim_anneal(cooling_coef = 0:1))
